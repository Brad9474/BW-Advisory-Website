...
const LeadCaptureForm = ({ values, onChange, onSubmit, submitting }) => {
  const required = ['name', 'organisation', 'role', 'email'];
  ...
  return (
    ...
    >
      ...
      <p className="text-xs text-silver/50 font-light">
        Your details are handled in line with our <a href="/privacy" className="underline hover:text-[#C9A84C]">privacy policy</a>. No spam.
      </p>
    </form>
  );
};
...
const Checkbox = ({ checked, onChange, label, required }) => (
  <label className="flex items-start gap-3 cursor-pointer group">
    ...
    />
    <span className="text-silver/85 font-light text-sm md:text-base group-hover:text-white transition-colors duration-300">
      {label}{required && <span className="text-[#C9A84C]"> *</span>}
    </span>
  </label>
);
