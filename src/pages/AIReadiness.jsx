...
// ─────────────────────────────────────────────────────────────────────────────
// Section A — shared across all sectors
// ─────────────────────────────────────────────────────────────────────────────
...
const getSector = (a1Label) => {
  if (HEALTHCARE_LABELS.has(a1Label)) return 'healthcare';
  if (PROFESSIONAL_LABELS.has(a1Label)) return 'professional';
  if (a1Label === 'Trades / construction') return 'trades';
  return 'retail'; // Retail, Hospitality, Other
};
...
const getDiagnosticType = (label) => {
  const map = {
    healthcare: 'SMB_HEALTHCARE',
    professional: 'SMB_PROFESSIONAL',
    trades: 'SMB_TRADES',
    retail: 'SMB_RETAIL',
  };
  return map[getSector(label)] ?? 'SMB_HEALTHCARE';
};

// ─────────────────────────────────────────────────────────────────────────────
// Sector-specific B1 (business management system)
// ─────────────────────────────────────────────────────────────────────────────
...
const B1_BY_SECTOR = {
  ...
  trades: {
    id: 'B1',
    text: 'Which job management or field service software do you use?',
    helper: 'Select all that apply.',
    kind: 'multi',
    allowOther: true,
    options: [
      'Tradify', 'simPRO', 'ServiceM8', 'Fergus', 'AroFlo', 'Buildxact',
      'Procore', 'Assignar', 'CoConstruct', 'BuilderTrend',
      'Monday.com', 'Asana', 'Notion',
      "We don't use a job management system", 'Not sure what this is',
    ].map((label) => ({ label })),
  },
  retail: {
    id: 'B1',
    text: 'Which retail, point-of-sale, or business management system do you use?',
    helper: 'Select all that apply.',
    kind: 'multi',
    allowOther: true,
    options: [
      'Lightspeed Retail', 'Vend', 'Shopify', 'Square', 'MYOB Retail Manager',
      'Revel Systems', 'Deputy', 'Kounta', 'DEAR Inventory', 'QuickBooks POS',
      'Monday.com', 'Asana', 'Notion',
      "We don't use a retail or POS system", 'Not sure what this is',
    ].map((label) => ({ label })),
  },
};
B1_BY_SECTOR.professional = B1_BY_SECTOR.healthcare;

// ─────────────────────────────────────────────────────────────────────────────
// Shared Section B questions (B2–B4, B6–B7) — same across all sectors
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Sector-specific B5 (customer/patient communication tools)
// ─────────────────────────────────────────────────────────────────────────────
...
const B5_BY_SECTOR = {
  ...
  trades: {
    id: 'B5',
    text: 'Which tools do you use to communicate with customers about jobs?',
    helper: 'Select all that apply.',
    kind: 'multi',
    allowOther: true,
    options: [
      'Automated job confirmations and updates via job management software',
      'Automated quote and invoice delivery',
      'Manual phone calls to customers',
      'Manual SMS sent by staff',
      'Manual emails sent by staff',
      'WhatsApp or similar for job coordination',
      "We don't have a structured customer communication process",
      'Not sure',
    ].map((label) => ({ label })),
  },
  retail: {
    id: 'B5',
    text: 'Which tools do you use to communicate with customers?',
    helper: 'Select all that apply.',
    kind: 'multi',
    allowOther: true,
    options: [
      'Email newsletters (MailChimp, Klaviyo, etc.)',
      'SMS marketing (via POS or dedicated platform)',
      'Loyalty program notifications',
      'Social media direct messages',
      'Manual calls or messages by staff',
      "We don't have structured customer communication",
      'Not sure',
    ].map((label) => ({ label })),
  },
};
B5_BY_SECTOR.professional = B5_BY_SECTOR.healthcare;

// ─────────────────────────────────────────────────────────────────────────────
// Section C — C1 and C5/C6 are shared; C2/C3/C4 are sector-specific
// ─────────────────────────────────────────────────────────────────────────────
...
const C234_BY_SECTOR = {
  ...
  trades: [
    {
      id: 'C2',
      text: 'How do you confirm job bookings and keep customers updated on scheduling?',
      kind: 'single',
      options: [
        { label: 'Fully automated — confirmations and updates sent without staff involvement', gap: 0 },
        { label: 'Partly automated — some steps handled manually', gap: 1 },
        { label: 'Manual — staff call or message each customer individually', gap: 3 },
        { label: 'We confirm jobs occasionally but inconsistently', gap: 3 },
        { label: "We don't send booking confirmations or job updates to customers", gap: 4 },
      ],
    },
    {
      id: 'C3',
      text: 'How do you follow up with customers after a job is completed?',
      kind: 'single',
      options: [
        { label: 'Automated — follow-up sent automatically', gap: 0 },
        { label: 'Manual — staff contact individually', gap: 2 },
        { label: 'Ad hoc — we do it sometimes, not consistently', gap: 3 },
        { label: "We don't follow up after jobs are completed", gap: 4 },
      ],
    },
    {
      id: 'C4',
      text: 'How do you handle new customer inquiries from first contact through to a confirmed job?',
      kind: 'single',
      options: [
        { label: 'Fully digital — online quotes, automated follow-up, digital sign-off', gap: 0 },
        { label: 'Mix of digital and manual steps', gap: 2 },
        { label: 'Mostly manual — phone calls and paper or emailed quotes', gap: 3 },
        { label: 'Staff manually track everything in spreadsheets or notebooks', gap: 4 },
        { label: "We're not sure of our exact process", gap: 2 },
      ],
    },
  ],
  retail: [
    {
      id: 'C2',
      text: 'How do you communicate promotions, new arrivals, or time-sensitive offers to existing customers?',
      kind: 'single',
      options: [
        { label: 'Fully automated — scheduled campaigns sent without manual effort', gap: 0 },
        { label: 'Partly automated — some setup required each time', gap: 1 },
        { label: 'Manual — staff prepare and send each communication individually', gap: 3 },
        { label: 'We communicate with customers occasionally but inconsistently', gap: 3 },
        { label: "We rely on foot traffic — no structured outbound communication", gap: 4 },
      ],
    },
    {
      id: 'C3',
      text: 'How do you follow up with customers after a purchase?',
      kind: 'single',
      options: [
        { label: 'Automated — follow-up or re-engagement message sent automatically', gap: 0 },
        { label: 'Manual — staff contact individually', gap: 2 },
        { label: 'Ad hoc — sometimes, not consistently', gap: 3 },
        { label: "We don't follow up after purchases", gap: 4 },
      ],
    },
    {
      id: 'C4',
      text: 'How do you capture new customer details and encourage repeat visits?',
      kind: 'single',
      options: [
        { label: 'Fully digital — loyalty or CRM system captures data automatically at point of sale', gap: 0 },
        { label: 'Mix of digital and manual capture', gap: 2 },
        { label: 'Mostly manual — paper sign-ups or verbal collection', gap: 3 },
        { label: "We don't systematically capture customer information", gap: 4 },
        { label: "We're not sure of our process", gap: 2 },
      ],
    },
  ],
};
C234_BY_SECTOR.professional = C234_BY_SECTOR.healthcare;

// ─────────────────────────────────────────────────────────────────────────────
// Section D — D3–D5, D7 are shared; D1, D2, D6 are sector-specific
// ─────────────────────────────────────────────────────────────────────────────
...
const D1_BY_SECTOR = {
  ...
  trades: {
    id: 'D1',
    text: 'How often do scheduling errors, double-bookings, or missed job appointments occur?',
    kind: 'single',
    options: [
      { label: 'Never or extremely rarely', gap: 0 },
      { label: 'Once or twice a month', gap: 1 },
  ...
  retail: {
    id: 'D1',
    text: 'How often do stock discrepancies, ordering errors, or inventory issues cause operational disruption?',
    kind: 'single',
    options: [
      { label: 'Never or extremely rarely', gap: 0 },
      { label: 'Once or twice a month', gap: 1 },
...
D1_BY_SECTOR.professional = D1_BY_SECTOR.healthcare;
...
const D2_BY_SECTOR = {
  ...
  trades: {
    id: 'D2',
    text: 'Have you ever lost a customer or piece of work because of an administrative or communication failure?',
    kind: 'single',
    options: [
      { label: "Not that I'm aware of", gap: 0 },
      { label: 'Once or twice', gap: 1 },
  ...
  retail: {
    id: 'D2',
    text: 'Have you ever lost a customer or piece of business because of an administrative or communication failure?',
    kind: 'single',
    options: [
      { label: "Not that I'm aware of", gap: 0 },
      { label: 'Once or twice', gap: 1 },
...
D2_BY_SECTOR.professional = D2_BY_SECTOR.healthcare;
...
const D6_BY_SECTOR = {
  ...
  trades: {
    ...
    options: [
      ...
      'Staff spending time on tasks below their skill level',
      "Systems that don't talk to each other",
      'Missed jobs, no-shows, or communication breakdowns',
      'Inconsistent customer or job delivery experience',
      "We're growing but our processes haven't kept up",
      'Dependent on one or two people who know how things work',
      "No visibility into what's actually happening",
      'Security or data protection — not sure how exposed we are',
      'Compliance — not confident we meet our obligations',
      'Quoting errors, billing disputes, or revenue leakage',
      'Something else',
    ].map((label) => ({ label })),
  },
  retail: {
    ...
    options: [
      ...
      'Staff spending time on tasks below their skill level',
      "Systems that don't talk to each other",
      'Stock shortages, ordering errors, or communication breakdowns',
      ...
      'Security or data protection — not sure how exposed we are',
      'Compliance — not confident we meet our obligations',
      'POS errors, refund issues, or revenue leakage',
      'Something else',
    ].map((label) => ({ label })),
  },
};
D6_BY_SECTOR.professional = D6_BY_SECTOR.healthcare;
...
// ─────────────────────────────────────────────────────────────────────────────
// Section E — E1 is sector-specific; E2/E3 are shared
// ─────────────────────────────────────────────────────────────────────────────
...
const E1_BY_SECTOR = {
  ...
  trades: {
    ...
    kind: 'multi',
    options: [
      'Take on more jobs — grow revenue',
      'Improve the quality of work and customer experience',
      'Reduce administrative or overhead costs',
      'Give existing staff more meaningful work',
      'Reduce my own hours and get out of the day-to-day',
      'Invest in business development and growth',
      'All of the above — any recovered time has value',
  ...
  retail: {
    ...
    kind: 'multi',
    options: [
      'Serve more customers — grow revenue',
...
E1_BY_SECTOR.professional = E1_BY_SECTOR.healthcare;
...
// All sectors produce 28 questions
...
// ─────────────────────────────────────────────────────────────────────────────
// Scoring — all functions accept activeQById (sector-aware question lookup)
// ─────────────────────────────────────────────────────────────────────────────

const gapFor = (questionById, qid, value) => {
...
const computeSectionGaps = (responses, questionById) => {
  const g = (qid, val) => gapFor(questionById, qid, val);
  const bGap = g('B6', responses.B6) + g('B7', responses.B7);
  const cGap =
    g('C1', responses.C1) + g('C2', responses.C2) + g('C3', responses.C3) +
    g('C4', responses.C4) + g('C5', responses.C5) + g('C6', responses.C6);
  const d6Count = Array.isArray(responses.D6) ? Math.min(responses.D6.length, 4) : 0;
  const dGap =
    g('D1', responses.D1) + g('D2', responses.D2) + g('D3', responses.D3) +
    g('D4', responses.D4) + g('D5', responses.D5) + d6Count + g('D7', responses.D7);
  const eGap = g('E2', responses.E2);
  return {
    bGap, cGap, dGap, eGap,
...
const computeScore = (responses, questionById) => {
  const { bNorm, cNorm, dNorm, eNorm } = computeSectionGaps(responses, questionById);
...
const SESSION_RATES = {
  ...
  'Retail': 80,
  'Trades / construction': 130,
  'Hospitality': 65,
};

const calcStaff = (responses, questionById) => {
  const a2Opt = questionById.A2?.options?.find((o) => o.label === responses.A2);
  return a2Opt?.staff ?? 3;
};

const calcTimeLost = (responses, questionById) => {
  const c1Opt = questionById.C1?.options?.find((o) => o.label === responses.C1);
...
const calcRevenueAtRisk = (responses, questionById) => {
  ...
  const staff = calcStaff(responses, questionById);
  const rate = SESSION_RATES[responses.A1] ?? 120;
...
const computeOpportunity = (responses, questionById) => ({
  timeLost: calcTimeLost(responses, questionById),
  revenueAtRisk: calcRevenueAtRisk(responses, questionById),
});
...
const computeRiskAreas = (responses, questionById) => {
  const gapNorm = (qid) => {
    const q = questionById[qid];
    const max = Math.max(...(q?.options || []).map((o) => o.gap ?? 0));
    if (max === 0) return 0;
    return gapFor(questionById, qid, responses[qid]) / max;
  };
  const avg = (arr) => (arr.length === 0 ? 0 : arr.reduce((a, b) => a + b, 0) / arr.length);
  const d6Count = Array.isArray(responses.D6) ? Math.min(responses.D6.length, 4) : 0;

  const systems  = avg([gapNorm('B6'), gapNorm('B7')]);
  const workflow = avg([gapNorm('C1'), gapNorm('C2'), gapNorm('C3'), gapNorm('C4'), gapNorm('C5'), gapNorm('C6')]);
  ...
  return [
    { id: 'systems',    label: 'Systems & Integration',          norm: systems,    severity: severityOf(systems) },
    { id: 'workflow',   label: 'Operational Workflow',            norm: workflow,   severity: severityOf(workflow) },
    { id: 'failures',   label: 'Recurring Operational Failures',  norm: failures,   severity: severityOf(failures) },
    { id: 'visibility', label: 'Visibility & Awareness',          norm: visibility, severity: severityOf(visibility) },
    { id: 'strategic',  label: 'Strategic Posture',               norm: strategic,  severity: severityOf(strategic) },
  ];
};

const REVIEW_TEMPLATES = {
  systems:    'Your systems are operating in isolation. Every workflow that needs data from more than one place creates manual effort, and the cost compounds across every working day.',
  workflow:   'Your team is spending significant time on manual administrative tasks that could be largely automated. This is the single biggest cost driver in your current operation.',
  failures:   'You are losing measurable revenue from administrative and communication failures. These are recoverable with the right process design — the cost is real and the fix is well-understood.',
  visibility: "You don't currently have a clear picture of where time is being lost or what it is costing. That is the first thing to fix — you cannot manage what you cannot measure.",
  strategic:  'Past attempts to improve your processes have not held. This is usually a sequencing and change-management problem, not a technology problem.',
};
...
const trackEvent = (payload) => {
  try {
    fetch(TRACK_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), keepalive: true }).catch(() => {});
  } catch { /* fire-and-forget */ }
};

const buildReferralToken = (email) => {
  try { return btoa((email || '').trim()).replace(/=/g, ''); } catch { return ''; }
};

const submitToCommandCentre = async (payload) => {
  try {
    const res = await fetch(API_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    if (!res.ok) { console.error('[ai-readiness] API responded', res.status, await res.text().catch(() => '')); return { ok: false }; }
    return { ok: true };
  } catch (err) { console.error('[ai-readiness] API submission failed', err); return { ok: false }; }
};
...
const fmtMoney = (n) => `$${(Math.round(n / 100) * 100).toLocaleString('en-AU')}`;
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
        <div className="h-full bg-[#C9A84C] transition-all duration-500 ease-out" style={{ width: `${pct}%` }} />
      </div>
    </div>
  );
};
...
const QuestionScreen = ({ question, value, otherValue, onChange, onOtherChange, onAdvance, onSelectionMade, isFirstScreen }) => {
  ...
  if (question.kind === 'text') {
    return (
      <div className="space-y-6">
        <h2 className="font-display font-bold text-2xl md:text-3xl text-white leading-snug">{question.text}</h2>
        ...
        />
        <p className="text-xs text-silver/50 font-mono">{(value || '').length}/{question.maxLength || 200}</p>
      </div>
    );
  }
  ...
  if (question.kind === 'multi') {
    ...
    const toggleOther = () => {
      if (otherChecked) { onChange(arr.filter((x) => x !== OTHER_LABEL)); if (onOtherChange) onOtherChange(''); }
      else { onChange([...arr, OTHER_LABEL]); }
    };
    return (
      <div className="space-y-6">
        <h2 className="font-display font-bold text-2xl md:text-3xl text-white leading-snug">{question.text}</h2>
        ...
        <div className="grid grid-cols-1 gap-3">
          {question.options.map((opt, i) => (
            <OptionCard key={opt.label} multi checked={arr.includes(opt.label)} label={opt.label} onClick={() => handleMultiToggle(opt.label)} autoFocus={i === 0 && !isFirstScreen} />
          ))}
          {question.allowOther && (
            <>
              <OptionCard multi checked={otherChecked} label={OTHER_LABEL} onClick={toggleOther} />
              {otherChecked && (
                <input type="text" value={otherValue || ''} onChange={(e) => onOtherChange && onOtherChange(e.target.value.slice(0, 200))} maxLength={200} placeholder="Enter software name"
          ...
          {question.id === 'E1' && arr.includes(SOMETHING_ELSE) && (
            <input type="text" value={otherValue || ''} onChange={(e) => onOtherChange && onOtherChange(e.target.value.slice(0, 200))} maxLength={200} placeholder="Tell us what you'd do with it — optional"
              className="w-full bg-white/5 border border-white/15 rounded-xl px-5 py-4 text-white placeholder:text-silver/40 focus:outline-none focus:border-[#C9A84C] transition-colors duration-200" />
          )}
          {question.id === 'D6' && arr.includes(SOMETHING_ELSE) && (
            <input type="text" value={otherValue || ''} onChange={(e) => onOtherChange && onOtherChange(e.target.value.slice(0, 200))} maxLength={200} placeholder="Tell us what the problem is — optional"
              className="w-full bg-white/5 border border-white/15 rounded-xl px-5 py-4 text-white placeholder:text-silver/40 focus:outline-none focus:border-[#C9A84C] transition-colors duration-200" />
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3">
        {question.options.map((opt, i) => (
          <OptionCard key={opt.label} checked={value === opt.label} label={opt.label} onClick={() => handleSingle(opt.label)} autoFocus={i === 0 && !isFirstScreen} />
        ))}
      </div>
      {showHealthcareFollowUp && (
        <div ref={followUpRef} className="space-y-3 pt-4 border-t border-white/10">
          <label className="block">
            <span className="text-silver/75 text-xs font-mono tracking-[0.3em] uppercase font-bold">Tell us more — what type of practice?</span>
            <input type="text" value={otherValue || ''} onChange={(e) => onOtherChange && onOtherChange(e.target.value.slice(0, 200))} placeholder="Optional — e.g. mixed billing GP, paediatric specialist, group dental"
      ...
      {question.id === 'E2' && value === SOMETHING_ELSE && (
        <div className="space-y-3 pt-4 border-t border-white/10">
          <input type="text" value={otherValue || ''} onChange={(e) => onOtherChange && onOtherChange(e.target.value.slice(0, 200))} placeholder="Tell us your goal — optional"
            className="w-full bg-white/5 border border-white/15 rounded-xl px-5 py-4 text-white placeholder:text-silver/40 focus:outline-none focus:border-[#C9A84C] transition-colors duration-200" />
        </div>
      )}
    </div>
...
const ConsentCheckbox = ({ checked, onChange, required, children }) => (
  <label className="flex items-start gap-3 cursor-pointer group">
    <input type="checkbox" checked={!!checked} onChange={(e) => onChange(e.target.checked)} className="mt-1 w-4 h-4 accent-[#C9A84C] cursor-pointer flex-shrink-0" />
...
const EmailCaptureScreen = ({ lead, onChange, onSubmit, submitting }) => {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (!valid || submitting) return; onSubmit(); }} className="space-y-6">
      <h2 className="font-display font-bold text-2xl md:text-3xl text-white leading-snug">Where should we send your results?</h2>
      <p className="text-silver/70 font-light text-base md:text-lg">Your score appears on the next screen.</p>
      <div className="space-y-4">
        <label className="flex flex-col gap-2">
          <span className="text-silver/75 text-xs font-mono tracking-[0.3em] uppercase font-bold">Name <span className="text-[#C9A84C]">*</span></span>
          <input autoFocus type="text" required value={lead.name} onChange={(e) => onChange('name', e.target.value)}
        ...
        <label className="flex flex-col gap-2">
          <span className="text-silver/75 text-xs font-mono tracking-[0.3em] uppercase font-bold">Email <span className="text-[#C9A84C]">*</span></span>
          <input type="email" required value={lead.email} onChange={(e) => onChange('email', e.target.value)}
            className="bg-white/5 border border-white/15 rounded-lg px-5 py-4 min-h-[48px] text-white text-base placeholder:text-silver/40 focus:outline-none focus:border-[#C9A84C] transition-colors duration-200" />
        </label>
      </div>
      <div className="space-y-4 pt-6 border-t border-white/10">
        <ConsentCheckbox checked={lead.consentContact} onChange={(v) => onChange('consentContact', v)} required>
        ...
        <ConsentCheckbox checked={lead.consentMarketing} onChange={(v) => onChange('consentMarketing', v)}>
      ...
      <div className="pt-4">
        <button type="submit" disabled={!valid || submitting}
          className={['w-full md:w-auto px-12 py-5 min-h-[48px] rounded-lg font-bold text-sm tracking-[0.15em] uppercase transition-all duration-300 inline-flex items-center justify-center gap-3',
            !valid || submitting ? 'bg-white/10 text-silver/40 cursor-not-allowed' : 'bg-[#C9A84C] text-[#0F172A] hover:bg-[#E0BC60] cursor-pointer shadow-[0_8px_24px_rgba(201,168,76,0.3)]'].join(' ')}>
...
const Results = ({ score, opportunity, riskAreas, review, lead, referralToken, diagnosticType }) => {
  ...
  const openMailto = () => {
    ...
    window.location.href = `mailto:?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(buildMailtoBody())}`;
  };

  const sendShare = () => {
    if (shareAttempted) { openMailto(); return; }
    setShareAttempted(true);
    const { name: referralName, email: referralEmail } = shareForm;
    if (referralName.trim() || referralEmail.trim()) {
      try {
        fetch(REFERRAL_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ referrerEmail: (lead?.email || '').trim(), referralName: referralName.trim(), referralEmail: referralEmail.trim(), score, diagnosticType }), keepalive: true }).catch(() => {});
      } catch { /* fire-and-forget */ }
    }
    openMailto();
  };
  ...
  return (
    <div className="space-y-12">
      <div className="bg-gradient-to-br from-white/12 via-white/6 to-white/3 backdrop-blur-sm border border-white/20 rounded-3xl p-8 md:p-14 text-center space-y-5">
        <p className="text-[#C9A84C] font-mono text-xs tracking-[0.3em] uppercase font-bold">Your Result</p>
        <h1 className="font-display font-bold text-3xl md:text-5xl text-white leading-tight">
          Your Operational Efficiency Score: <span className="text-[#C9A84C]">{score}/100</span>
        </h1>
        <p className="text-base md:text-lg text-silver/80 font-light leading-relaxed max-w-3xl mx-auto">
          Based on your responses, we've identified <span className="text-4xl font-bold text-[#C9A84C]">{concernCount}</span> area{concernCount === 1 ? '' : 's'} where your current workflows are creating measurable{' '}
          <span className="text-[#C9A84C]">inefficiency</span>, <span className="text-[#C9A84C]">duplication</span>, and <span className="text-[#C9A84C]">risk</span>.
        </p>
      </div>
      ...
      <div>
        <p className="text-[#C9A84C] font-mono text-xs tracking-[0.3em] uppercase font-bold mb-6">Risk Area Breakdown</p>
        <div className="grid grid-cols-1 gap-3">
          {riskAreas.map((a) => (
            <div key={a.id} className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 bg-white/5 border border-white/15 rounded-xl px-5 md:px-6 py-4">
              <p className="text-white font-display font-semibold text-lg">{a.label}</p>
              <span className={`self-start md:self-auto text-[11px] font-mono font-bold tracking-[0.3em] uppercase px-3 py-1 rounded border ${SEVERITY_BADGE[a.severity]}`}>{a.severity}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/15 rounded-2xl p-6 md:p-8 space-y-3">
        <p className="text-silver/60 font-mono text-xs tracking-[0.3em] uppercase font-bold">Here's what stood out in your responses</p>
      ...
      <div className="relative bg-gradient-to-br from-white/8 via-white/4 to-white/2 backdrop-blur-xl border border-[#C9A84C]/30 rounded-3xl p-8 md:p-14 space-y-8">
        ...
        {import.meta.env.VITE_PURCHASE_SURFACE_ENABLED === 'true' ? (
          ...
          <p className="text-silver/70 font-light text-sm leading-relaxed text-center max-w-lg mx-auto">
            Paid reports aren't available yet. Book a call with Brad and he'll walk through your results directly.
          </p>
        )}
      ...
      <div className="space-y-4">
        <div className="flex flex-col md:flex-row gap-3">
          ...
          <button type="button" onClick={() => setShowShareForm((s) => !s)}
            className="flex-1 px-6 py-4 min-h-[48px] rounded-lg border border-[#C9A84C]/60 text-[#C9A84C] font-bold text-sm tracking-[0.15em] uppercase hover:bg-[#C9A84C]/10 hover:border-[#C9A84C] transition-all duration-200 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A84C]/50">
            Share results
          </button>
        </div>
        {showShareForm && (
          <div className="bg-white/5 border border-white/15 rounded-2xl p-6 space-y-4">
            {shareAttempted ? (
              <p className="text-silver/85 font-light text-base text-center py-4">Results shared. Check your email for a copy.</p>
            ) : (
              <>
                <p className="text-silver/80 font-light text-base">Who are you sharing this with?</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {[['name', 'Name (optional)'], ['email', 'Email (optional)']].map(([field, lbl]) => (
                    <label key={field} className="flex flex-col gap-2">
                      <span className="text-silver/75 text-xs font-mono tracking-[0.3em] uppercase font-bold">{lbl}</span>
                      <input type={field === 'email' ? 'email' : 'text'} value={shareForm[field]}
                        onChange={(e) => setShareForm((prev) => ({ ...prev, [field]: e.target.value.slice(0, 200) }))}
                ...
                <button type="button" onClick={sendShare}
                  className="px-8 py-3 min-h-[44px] rounded-lg bg-[#C9A84C] text-[#0F172A] font-bold text-sm tracking-[0.15em] uppercase hover:bg-[#E0BC60] transition-all duration-300 cursor-pointer shadow-[0_8px_24px_rgba(201,168,76,0.3)]">
                  Send
                </button>
              </>
            )}
...
const AIReadiness = () => {
  ...
  const [sessionId] = useState(() => {
    ...
    catch { return `s_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`; }
  });
  ...
  useEffect(() => {
    if (responses.A1 === undefined) { prevA1Ref.current = undefined; return; }
    if (prevA1Ref.current !== undefined && prevA1Ref.current !== responses.A1) {
      setResponses((prev) => {
  ...
  const scrollNextIntoView = useCallback(() => {
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        if (nextButtonRef.current) nextButtonRef.current.scrollIntoView({ behavior: 'smooth', block: 'end' });
      });
    });
  }, []);
  ...
  const submitLead = async () => {
    setSubmitting(true);
    const score        = computeScore(responses, activeQById);
    const opportunity  = computeOpportunity(responses, activeQById);
    const riskAreas    = computeRiskAreas(responses, activeQById);
    ...
    submitToCommandCentre(payload).finally(() => setSubmitting(false));
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      if (step === STEP_INTRO) { startDiagnostic(); return; }
  ...
  }, [step, responses, advance, startDiagnostic, activeQuestions]);
    ...
    screenContent = (
      <div className="space-y-8">
        <SectionHeader
          ...
          totalSections={buildSections(sector).length}
  ...
  const currentQ      = step < TOTAL_QUESTIONS ? activeQuestions[step] : null;
  const singleNeedsNext = currentQ?.kind === 'single' && singleSelectNeedsManualNext(currentQ.id, responses[currentQ.id]);
  const showNextButton  = step < TOTAL_QUESTIONS && currentQ && (currentQ.kind === 'multi' || currentQ.kind === 'text' || singleNeedsNext);
        ...
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          {(showBack || showNextButton) && (
            <div className="flex items-center justify-between gap-4">
              <button type="button" onClick={goBack} disabled={!showBack}
                className={['text-silver/70 font-mono text-xs tracking-[0.15em] uppercase font-bold transition-colors duration-200 px-4 py-3 min-h-[44px]',
                  !showBack ? 'opacity-0 pointer-events-none' : 'hover:text-white cursor-pointer'].join(' ')}>
                ← Back
              </button>
              {showNextButton ? (
                <button ref={nextButtonRef} type="button" onClick={advance} disabled={!canGoNext}
                  className={['px-8 py-4 min-h-[48px] rounded-lg font-bold text-sm tracking-[0.15em] uppercase transition-all duration-300 inline-flex items-center gap-3',
                    canGoNext ? 'bg-[#C9A84C] text-[#0F172A] hover:bg-[#E0BC60] cursor-pointer shadow-[0_8px_24px_rgba(201,168,76,0.3)]' : 'bg-white/10 text-silver/40 cursor-not-allowed'].join(' ')}>
                  Next
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                </button>
              ) : <span />}
