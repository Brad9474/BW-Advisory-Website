...
export default function IntelligenceCanvas() {
  ...
  useEffect(() => {
    ...
    // Degree → colour per spec
    function nodeColourRGB(node) {
      const td = node.targetDegree;
      if (td >= 7) return [200,  30,  20];
      if (td >= 5) return [230,  80,  20];
    ...
    function buildNodes() {
      ...
      const tryAdd = (xF, yF, attrs) => {
        ...
        nodes.push({
          xF, yF,
          activatedAt: null, firstActivatedAt: null,
          degree: 0, isHub: false, isSuperHub: false,
          cluster: -1, targetDegree: 0,
          ...attrs,
        });
        return true;
      };

      // 5 hubs at well-distributed fixed positions
      const hubs = [
        [0.12, 0.20, 5], [0.85, 0.18, 5],
        [0.08, 0.70, 4], [0.88, 0.66, 4],
        [0.50, 0.50, 4],
      ];
      hubs.forEach(([x, y, deg]) =>
        tryAdd(x, y, { isHub: true, targetDegree: deg })
      );

      // Fill remaining 17 nodes randomly
      let attempts = 0;
      while (nodes.length < 22 && attempts < 3000) {
        attempts++;
        tryAdd(rn(0.05, 0.95), rn(0.05, 0.95), { targetDegree: 2 });
      }

      // Top 2 hubs by targetDegree become superHubs
    ...
    function pickEdgeFor(g) {
      ...
      if (toIdx === -1) {
        ...
        if (candidates.length === 0) {
          ...
          toIdx = unvisitedHubs.length > 0
            ? nearestInPool(fromIdx, unvisitedHubs)
            : nearestInPool(fromIdx, candidates);
        } else {
    ...
    function maybeSpawnNewGrower() {
      ...
      for (const i of candidates) {
        ...
        for (const tip of tips) {
          const dx = nodes[i].xF - nodes[tip].xF, dy = nodes[i].yF - nodes[tip].yF;
          const d = dx * dx + dy * dy;
    ...
    function drawNode(i, t) {
      ...
      if (!isActive) {
        ...
        if (n.isHub) {
          ...
          CX.beginPath(); CX.arc(x, y, sz * 2.5, 0, Math.PI * 2);
      ...
      if (n.isSuperHub) {
        ...
        CX.beginPath(); CX.arc(x, y, bloomR, 0, Math.PI * 2);
      ...
      gg.addColorStop(0, col(glowA)); gg.addColorStop(1, col(0));
      CX.beginPath(); CX.arc(x, y, baseGlow * pulseScale, 0, Math.PI * 2);
      CX.fillStyle = gg; CX.fill();

      if (n.isHub) {
        const coreSzBase = n.isSuperHub ? 8 : 5.2;
        const pulse = 0.5 + 0.5 * Math.sin(t / HUB_RING_PERIOD * Math.PI * 2);
        CX.beginPath(); CX.arc(x, y, coreSzBase * 2.5, 0, Math.PI * 2);
        CX.strokeStyle = col(0.35 * pulse);
        CX.lineWidth = 1; CX.stroke();
      }

      const coreSz = Math.min(pulseScale, 1.8) * (n.isSuperHub ? 8 : n.isHub ? 5.2 : 3.5);
      CX.beginPath(); CX.arc(x, y, coreSz, 0, Math.PI * 2);
      ...
      if (phase === 'drawing') {
        for (const g of growers) {
          ...
          if (prog >= 1) {
            ...
            if (wasNew) {
              visitedSet.add(dst); visitedOrder.push(dst);
              nodes[dst].firstActivatedAt = t;
            }
            nodes[dst].activatedAt = t;
            nodes[fromIdx].degree++; nodes[dst].degree++;
